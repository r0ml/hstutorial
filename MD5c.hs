...
stringDigest x = BC.unpack (unsafePerformIO $ B.useAsCStringLen x f)
  where f (p,x) = B.create (2*x) (\z -> c_hexify (castPtr z) (fromIntegral x) p)

foreign import ccall unsafe "hexify" c_hexify :: CString -> CInt -> CString -> IO ()
